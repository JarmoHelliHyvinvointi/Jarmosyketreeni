...
function updateDurationsFromTotal() {
    ...
    // 20% lämmittely+jäähdyttely, 80% huiput+lepojaksot
    const warmupCooldownSeconds = Math.floor(totalSeconds * 0.20);
    ...
    const totalCycles = peakCount + (peakCount - 1) * 0.75;
    ...
    // Päivitä UI
    const warmEl = document.getElementById('warmupTime');
    const coolEl = document.getElementById('cooldownTime');
    const totalEl = document.getElementById('totalWorkoutTime');
    if (warmEl) warmEl.value = workoutConfig.warmupTime;
    if (coolEl) coolEl.value = workoutConfig.cooldownTime;
    if (totalEl) totalEl.value = `${totalMinutes}:00`;
}
...
// Save profile (tuotu puhtaana takaisin, ei debugia)
...
// Calculate total workout time (käytetään lähinnä näytössä, suunnitelma laskee itse)
...
// Start workout + esikatselu overlaynä
...
function initResultsChart() {
    ...
    if (resultsChart.plugins && typeof resultsChart.plugins.register === 'function') {
        resultsChart.plugins.register(backgroundPlugin);
        resultsChart.update();
    } else if (typeof Chart !== 'undefined' && typeof Chart.register === 'function') {
        Chart.register(backgroundPlugin);
        resultsChart.update();
    } else {
        console.warn('Taustapluginia ei voitu rekisteröidä (Chart.js-versio).');
    }
}
...
// ---------- Alustus: ei enää window.onloadia ----------
...
// Käynnistä initApp joko heti tai DOMContentLoadedin jälkeen
