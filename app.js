...
// Kuinka kauan esikatselu näytetään (millisekunteina)
...
// KORJATTU: Start workout + esikatselu
function startWorkout() {
    ...
    if (!peakCountInput || !workoutDurationInput || !workoutScreenEl || !previewScreenEl) {
    ...
    // Näytä ESINÄKYMÄ
    hideAllScreens();
    previewScreenEl.classList.remove('hidden');

    // Alusta esikatselugraafi pienen viiveen jälkeen
    setTimeout(() => {
        initPreviewChart();
    }, 300);

    // Kun esikatselu on ollut näkyvissä hetken, siirrytään varsinaiseen treeninäkymään
    setTimeout(() => {
        hideAllScreens();
...
function initResultsChart() {
    ...
    // HUOM: tämä tapa toimii, jos käyttämäsi Chart.js-versio tukee plugins.register-metodia.
    // Jos tulee virhe "register is not a function", pluginin voi rekisteröidä Chart.register(backgroundPlugin) -tyyliin.
    if (resultsChart.plugins && typeof resultsChart.plugins.register === 'function') {
        resultsChart.plugins.register(backgroundPlugin);
        resultsChart.update();
    } else {
        // Varatoiminto: ei taustavärejä, mutta kuvaaja toimii.
...
function downloadResults() {
    ...
    a.href = url;
    a.download = `treeni_${new Date().toISOString().slice(0, 